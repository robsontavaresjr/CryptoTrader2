# -*- coding: utf-8 -*-
"""
Created on Wed Apr 27 10:26:59 2016
...
class SlaveTrader_ID:
    
    def __init__(self, strategy, initcash, database,working_days):
    ...
    def feed_workmemory(self, workmemory, ticker, stock_data, current_date):
        ...
        # Into the WM
        if workmemory[ticker].has_key('INVALID_DATE'):
    ...
    def inference_engine(self, workmemory, ticker):
        ...
        if not workmemory[ticker].has_key('INVALID_DATE'):
    ...
    def expected_value(self,briefedPortfolio = True):
        ...
        if self.sell_count == 0:
            ...
            for eachStock in self.operation_log.keys():
                ...
                weight    = np.array([np.exp(-i) for i in np.arange(0.5,1.,.1)])
                boxes     = np.round(np.arange(1,len(trades),(len(trades)-1)/5.))
...
def backtest_simulation_acquisition(strategy_or_location, stocks, cash, interval, daysBack,
                                    risk_function,
                                    brokerfee=14.9,
                                    leverage=1.,
                                    interest=1.19,
    ...
    ############################################################################################################
    database = data_handler_ID.database_builder(stocks,interval, daysBack)
    indicators, working_days = data_handler_ID.calculate_indicators(stocks, database,indicator_filter = strategy_struct[0])
    
    print 'Indicators calculated.'

    workmemory = data_handler_ID.workmemory_builder(stocks)
    ############################################################################################################

    # Gera entidade SlaveTrader para operar

    trader = SlaveTrader_ID(strategy_struct, cash, indicators, working_days)
    ...
    stop_flag    = False
    period_list  = [working_days[working_days.keys()[0]][0]]
    end_op       = working_days[working_days.keys()[0]][-1]

    while not stop_flag:
        
        next_period = period_list[-1] + datetime.timedelta(seconds = interval)
        period_list.append(next_period)

        if next_period >= end_op:
            stop_flag = True
        else:
            pass

    period_list = period_list[199:]
    ############################################################################################################
    print 'Simulating ... please wait.'
    for current_date in period_list:
        ############################################################################################################
        if plot_simulation:
            print 'Simulating date: ', current_date
        workmemory = data_handler_ID.workmemory_feeder(workmemory, trader, current_date,indicator_filter = strategy_struct[0])  # Updates WM
    ...
    if plot_cashflow:
        ...
        print '=' * 50
        print ''
        print "Inicio de operacao em: ", period_list[0]
        print ''
...
def BacktestLoader(strategy_location, stocks, cash, database_or_location, operation_range, risk_function,
    ...
    if type(database_or_location) == str:
        database = data_handler_ID.database_loader(database_or_location)
    else:
        database = database_or_location

    indicators, working_days = data_handler_ID.calculate_indicators(stocks, database)
    print 'Indicators calculated.'
    ...
    for current_date in day_list:
        ...
        workmemory = data_handler_ID.workmemory_feeder(workmemory, trader, current_date)  # Updates WM
